...
========================
Build the 600-sample dataset decks: for each of the 12 hood designs, take the
design's first delivered deck (runs 1, 6, 11, ..., 56) and reposition the
rigid headform to each of the 50 Euro-NCAP grid locations produced by
euroncap_grid.py.
...
Numbering: run = 50*design + loc  (design 0..11, loc 1..50 -> runs 1..600),
mirroring the delivered convention (design = (run-1)//50).

Usage (from the Code/ directory):
    # everything (600 decks, ~4.8 GB) -- intended to run ON DELTA
...
def main():
    ...
    ap.add_argument("--locs", default="1-50", help="e.g. '1-50' or '1,2'")
    ...
    locations = [l for l in load_locations(args.locations) if l[0] in locs_wanted]
    ...
    for d in designs:
        ...
        print("   design %2d done (%d decks%s)"
              % (d, len(locations), ", dry-run" if args.dry_run else ""))

    manifest.sort(key=lambda r: r["run"])
    man_csv = os.path.join(args.out_dir, "manifest_600.csv")
    ...
    if len(manifest) and not args.dry_run:
        cc_csv = os.path.join(args.out_dir, "ImpactCoords_600.csv")
