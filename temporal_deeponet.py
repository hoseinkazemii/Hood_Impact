...
class HoodImpactNeuralOperator(nn.Module):
    ...
    def forward(
        ...
        """
        Returns:
            acceleration: (total_time_points,)
        """

        branch = self.mesh_encoder(mesh, mesh_batch, indentor, batch_size)
        branch = self.film_layer1(branch, indentor)
        branch = self.film_layer2(branch, indentor)
        branch = self.branch_transform(branch)

        if self.config.prediction_target == "hic":
            # Directly regress from branch features
...
def main():
    ...
    if config.data_format == "industrylike":
        # 60 designs (1 run each): last 10 -> test (runs 51-60),
        # 5 -> val (runs 46-50), first 45 -> train (runs 1-45).
        TEST_DESIGNS = list(range(50, 60))
        VAL_DESIGN = list(range(45, 50))
    else:
